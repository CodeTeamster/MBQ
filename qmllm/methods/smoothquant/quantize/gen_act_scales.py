...
def get_act_scales(model, prompt_inputs, prompt_kwargs):
    ...
    def stat_tensor(name, tensor):
        # hidden_dim [B, N, hidden_dim]
        hidden_dim = tensor.shape[-1]
        # tensor [B x N, hidden_dim]
    ...
    for name, m in model.model.named_modules():
        if (
            ...
            and "mlp1" not in name
        ):
            # print(f"register hook for {name}")
            hooks.append(
                # hook function will be triggered after forward
                m.register_forward_hook(functools.partial(stat_input_hook, name=name))
            )
        else:
            pass
            # print(f"skip register hook for {name}")

    num_samples = len(next(iter(prompt_inputs.values())))

    inputs = process_input(prompt_inputs, prompt_kwargs)

    for i in tqdm(range(num_samples), desc="Getting activation scales"):
        mini_inputs = {}
        for k in inputs:
            if isinstance(inputs[k], torch.Tensor):
                mini_inputs[k] = inputs[k][i:i+1].to(device)
